...
export function CompareView({
  ...
  const channelDisabled: Record<Channel, boolean> = {
    all: false,
    meta: false,
    google: selectedComps.some((c) => !c.google_advertiser_id && !c.google_domain),
    instagram: selectedComps.some((c) => !c.instagram_username),
  };
        ...
        <CardContent>
          <div className="flex gap-2 flex-wrap">
            {([
              ...
              const disabled = selected.size >= 2 && channelDisabled[ch.key];
              return (
                <Button
                  key={ch.key}
                  ...
                  title={disabled ? t("compare", "channelDisabledHint") : undefined}
                >
                  {ch.icon}
                  {ch.label}
                </Button>
          ...
          {selected.size >= 2 && (channelDisabled.google || channelDisabled.instagram) && (
            <p className="text-[10px] text-muted-foreground mt-2">
              {t("compare", "channelDisabledExplain")}
            </p>
          )}
        </CardContent>
      </Card>
